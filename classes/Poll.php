...
class Poll extends \ElggObject {
    ...
    public function vote($answers, $user_guid = 0) {
        ...
        $existing_votes = $this->getVote(false, $user_guid, true);
        ...
        if ($existing_votes) {
            ...
            if ($nbExistingVote < $nbAnswer) {
                // update existing
                foreach ($existing_votes as $key => $existing_vote) {
                    $annotate = elgg_get_annotation_from_id($key);
     ...
     */
    public function getVote($value_only = true, $user_guid = 0, $multi_value = false) {
        ...
        if ($multi_value) {
            ...
            foreach ($annotations as $annotation) {
                $res[$annotation->id] = $annotation->value;
            }
            return $res;
        } else {
